...
# body_index = {"left_knee": 1, "right_knee": 2, "left_ankle":3, "right_ankle":4}
...
while cap.isOpened():
    ...
    if success:
        ...
        # rounded_results=np.round(results[0].keypoints.numpy(),1)
        rounded_results = results[0].keypoints.numpy()
        data=results[0].keypoints.data.numpy()
       
        
        try:
            data=results[0].keypoints.data.numpy()
            left_knee = data[0][body_index["left_knee"]]
            right_knee = data[0][body_index["right_knee"]]
            left_ankle = data[0][body_index["left_ankle"]]
            right_ankle = data[0][body_index["right_ankle"]]
