...
class DoubleDribbleDetector:
    ...
    def process_frame(self, frame):
        ...
        rounded_results = pose_results[0].keypoints.numpy()
        data=pose_results[0].keypoints.data.numpy()
        # rounded_results = pose_results[0].keypoints.numpy().astype(np.float32).round(1)
        # rounded_results = np.round(pose_results[0].keypoints.numpy().astype(np.float32), 1)

        # rounded_results = np.array([np.round(p, 1) for p in pose_results[0].keypoints.numpy()])
        # rounded_results = np.array([np.round(coord, 1) for pose in pose_results[0].xyxy for coord in pose])

        try:
            # Extract the coordinates of the wrists.
            data=pose_results[0].keypoints.data.numpy()
            left_wrist = data[0][10]
            right_wrist = data[0][9]
